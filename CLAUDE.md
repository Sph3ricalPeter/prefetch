...
8. **Always verify CI after pushing the tag.** Run `gh run list --limit 3` and wait for both the CI and Release workflows to complete successfully. If either fails, check `gh run view <id> --log-failed`, fix the issue, and re-tag before reporting the release as done. A release is not complete until CI is green.
